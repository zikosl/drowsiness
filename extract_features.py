...
def extractor(image_path):
    ...
    with tf.Session() as sess:
        for op in sess.graph.get_operations():
            print(op.name)
        pooling_tensor = sess.graph.get_tensor_by_name('module_apply_default/InceptionV3/Logits/GlobalPool:0')
        image_data = tf.gfile.FastGFile(image_path, 'rb').read()
        pooling_features = sess.run(pooling_tensor, {'module_apply_default/hub_output/feature_vector/SpatialSqueeze:0': image_data})
...
def extract_features():
    with open('data/data_file.csv','r') as f:
        ...
        for videos in reader:
            path = os.path.join('data', 'sequences', videos[2] + '-' + str(26) + '-features.npy')
